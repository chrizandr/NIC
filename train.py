from data_evaluation import DataEvaluator
...
model.fit_generator(generator=generator.flow(mode='train'),
                    steps_per_epoch=int(generator.training_dataset.shape[0] / BATCH_SIZE),
                    epochs=NUM_EPOCHS,
                    verbose=1,
                    callbacks=callbacks,
                    validation_data=generator.flow(mode='validation'),
                    validation_steps=int(generator.validation_dataset.shape[0] / BATCH_SIZE))
