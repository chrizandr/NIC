...
class DataHandler():
    ...
    def __init__(self, captions_file, max_captions_len=20, word_freq_thresh=2,
                 if_extract_feature=False, image_dir=None, log_path=None,
                 separator='*'):
    ...
    def load_preprocess(self):
        ...
        for i in range(self.im_files.shape[0]):
            ...
            if (len(cleaned_caption) <= self.max_captions_len):
        ...
        print('Current number of files:', curr_size)
        ...
        if self.if_extract_feature:
            self.extract_im_features(self.image_dir)
    ...
    def remove_rare_words(self):
        ...
        for i, freq_data in enumerate(self.word_frequencies):
            freq = freq_data[1]
            if freq <= self.word_freq_thresh:
                index_range = i
                break

        if self.word_freq_thresh != 0:
